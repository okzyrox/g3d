...
function cameras:setCurrent(new_cam)
    cameras._current = new_cam

    new_cam:updateProjectionMatrix()
    new_cam:updateViewMatrix()
end
...
function camera:new()
    ...
    self.viewMatrix = Matrix.newMatrix()
    self.projectionMatrix = Matrix.newMatrix()

    -- set the global camera to this camera.
    if(not cameras:getCurrent()) then
        cameras:setCurrent(self)
    end
...
function camera:setLocked(state)
...
-- use this in your love.keypressed function, passing in the key
